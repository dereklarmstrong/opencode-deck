...
Feature requests and bugs land here as issues — open one before a PR for
anything beyond a clearly-scoped slice. Label on triage: one type label
(`bug` / `enhancement`) plus a scope label when the work clearly lives on
one side.
...
| `frontend` | `index.html` — UI, charts, tables, theming |
| `backend` | `server.py` — API, scan, tests |
| `docs` | README / CONTRIBUTING / PLAN (no scope label needed) |
| `wontfix` | declined request — see "Not planned" below |
