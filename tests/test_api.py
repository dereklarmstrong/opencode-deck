...
def build_fixture(path):
    ...
    The "today" turn is pinned to a moment that is always today AND strictly in
    the past (now - 5min, or now - 2s if we're in the first minute after
    midnight), so windowed queries clamped to `now` never exclude it regardless
    ...
    # Robust "recent today" instant: today and strictly < now.
    _midnight = today.replace(hour=0, minute=0, second=0, microsecond=0)
    if (today - _midnight) > timedelta(minutes=1):
        t_today = today - timedelta(minutes=5)
    else:
        t_today = today - timedelta(seconds=2)
    ...
    sessions = [
        ...
        ("s4", None, "/home/derek", "Fixture today", "build", model, 1000, 50, 0, 0, 0, _ts(t_today), _ts(t_today + timedelta(minutes=5))),
    ]
    ...
    # T6: today's turn (s4) — needed for current streak (recent-today, always in the past)
    t6 = _ts(t_today)
    ...
    conn.commit()
    conn.close()
    return {"today": today, "d1": d1, "d2": d2, "s1": "s1", "t_today": t_today}
...
class TestDashboard(unittest.TestCase):
    ...
    def test_hourly(self):
        d = self._all()
        t6 = self.fx["t_today"]
        self.assertEqual(d["hourly"][t6.weekday()][t6.hour], 1)  # T6 (s4)
        ...
        try:
            ...
            with mock.patch.object(server, "RESCAN_MIN_S", 3600):
                with mock.patch.object(server, "scan", wraps=server.scan) as m:
                    ...
                    t = _ts(datetime.now() - timedelta(seconds=5))  # recent past, in window
